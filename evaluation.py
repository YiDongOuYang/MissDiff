...
class FeatureMaker:
    ...
    def __init__(self, metadata, label_column=3, label_type='int', sample=50000):
...
def _prepare_ml_problem(train, val, test, metadata): 
    ...
    x_trains, y_trains = fm.make_features(train)
    # x_trains, y_trains = [], []
    # for i in train:
...
def _evaluate_multi_classification(train, test, fake, metadata):
    ...
    for model_spec in classifiers:
        ...
        for param in tqdm(param_set):
            ...
            # 3. auroc
            size = [a["size"] for a in metadata["columns"] if a["name"] == 3][0]
